...
pub async fn create_schedule(
    ...
    // Parse full datetime: "2025-11-11T07:40"
    let start_time: NaiveDateTime =
        match NaiveDateTime::parse_from_str(&payload.start_time, "%Y-%m-%dT%H:%M") {
            Ok(dt) => dt,
            Err(_) => {
                return (
    ...
    println!("Parsed start_time: {:?}", start_time);

    // Duration → end_time
    let duration_secs = payload.ad_duration.unwrap_or(30);
    ...
    println!("all values set, executing query...");
    println!("start_time: {}, end_time: {}", start_time, end_time);

    let result = sqlx::query!(
        r#"
        INSERT INTO ad_schedules 
        (id, ad_id, player_id, start_time, end_time, is_filler, ad_order, repeat_interval)
        VALUES ($1, $2, $3, $4::text::timestamp, $5::text::timestamp, $6, $7, $8::text::interval)
        "#,
        schedule_id,
        payload.ad_id,
        payload.player_id,
...
pub async fn delete_schedule(
    ...
    println!("Delete schedule result: {:?}", result);

    match result {
        Ok(_) => (
            axum::http::StatusCode::OK,
            format!("Schedule deleted with id: {}", schedule_id),
        )
            .into_response(),
        Err(e) => (
            axum::http::StatusCode::INTERNAL_SERVER_ERROR,
