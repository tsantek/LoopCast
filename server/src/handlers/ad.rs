...
#[axum::debug_handler]
pub async fn get_all_ads(
    State(state): State<Arc<AppState>>,
) -> axum::response::Result<axum::Json<Vec<Ad>>, axum::http::StatusCode> {
    let ads = sqlx::query!("SELECT id, name, file_name, duration, ad_type FROM ads")
        .fetch_all(&state.db)
        .await
        .map_err(|err| {
            eprintln!("Failed to fetch ads: {:?}", err);
            axum::http::StatusCode::INTERNAL_SERVER_ERROR
        })?;

    let ad_list: Vec<Ad> = ads
        .into_iter()
        .map(|ad| Ad {
            id: ad.id,
            name: ad.name,
            file_name: ad.file_name,
            duration: Some(ad.duration),
            ad_type: ad.ad_type,
        })
        .collect();

    Ok(axum::Json(ad_list))
}

#[axum::debug_handler]
pub async fn delete_ad(
    State(state): State<Arc<AppState>>,
    axum::extract::Path(ad_id): axum::extract::Path<uuid::Uuid>,
) -> impl IntoResponse {
    // First, get the file name from the database
    let ad = sqlx::query!("SELECT file_name FROM ads WHERE id = $1", ad_id)
        .fetch_one(&state.db)
        .await
        .map_err(|err| {
            eprintln!("Failed to fetch ad: {:?}", err);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;

    let file_path = format!("./uploads/{}", ad.file_name);
    ...
    sqlx::query!("DELETE FROM ads WHERE id = $1", ad_id)
        .execute(&state.db)
        .await
        .map_err(|err| {
            eprintln!("Failed to delete ad from database: {:?}", err);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
